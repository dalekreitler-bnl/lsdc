...
def setGovRobot(state):
  if state == "DI":
    ...
    if state in ["SA","DA"]:
      cameraThread = Thread(target=toggleLowMagCameraSettings,args=(state,))
      cameraThread.start()
    waitGov()
    robotGovState = (getPvDesc("robot%sActive" % altName) or getPvDesc("human%sActive" % altName))
    logger.info("robot gov state = " + str(robotGovState))
    if (robotGovState != 0):
      if cameraThread: cameraThread.join()
      return 1
    else:
...
def setGovRobotDI(): # keep this because it is different from the others
  setRobotGovState("DI")
  cameraThread = Thread(target=toggleLowMagCameraSettings,args=("DI",))
  cameraThread.start()
  waitGov()    
...
def logMxRequestParams(currentRequest):
  ...
  try: #I'm worried about unforseen ispyb db errors
    ...
    Reduced this sleep to 1 sec to speed up transitions, seems OK -DFK
    """
    time.sleep(1)  
